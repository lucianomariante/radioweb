...
namespace Tp\Notices\TPAEBlackfridayBanner;
...
if ( ! class_exists( 'Tpae_Blackfriday_Banner' ) ) {
	...
	class Tpae_Blackfriday_Banner {
		...
		public function __construct() {
			...
			if ( ! get_option( 'tpae_bfsale_notice_dismissed' ) ) {
				add_action( 'admin_notices', array( $this, 'tpae_bf_sale_banner' ) );
			}

			/** TPAE Bf Banner Close*/
			add_action( 'wp_ajax_tpae_blackfriday_dismiss_notice', array( $this, 'tpae_blackfriday_dismiss_notice' ) );
		}
		 ...
		 * @since 6.0.4
		 */
		public function tpae_bf_sale_banner() {

			$nonce  = wp_create_nonce( 'tpae-blackfriday-banner' );
			...
			$desc_text   = __( 'Our Black Friday Sale is live! Upgrade this season and get upto 50% OFF on the pro version.', 'tpebl');
			...
			if ( defined( 'THEPLUS_VERSION' ) ) {
				...
				$notice_text = __( 'Upgrade to Lifetime Unlimited Sites Plan and Save $120 Today! - Black Friday 50% OFF', 'tpebl');
			...
			echo '<div class="notice tpae-notice-show tpae-bf-banner is-dismissible" style="border-left: 4px solid #DF241B;">
				<div class="inline" style="display: flex;column-gap: 12px;align-items: center;padding: 15px 10px;position: relative;    margin-left: 0px;">
					<img style="max-width:136px;max-height:136px;" src="' . esc_url( L_THEPLUS_URL . '/assets/images/bf-banner.png' ) . '" />
					<div style="margin: 0 10px; color:#000;display:flex;flex-direction:column;gap:10px;">  
						...
						<div class="tpae-bf-btn" style="display:flex;column-gap:10px;flex-wrap:wrap;margin-top:3px;">
			...
			echo '<style> .notice.tpae-notice-show.tpae-bf-banner a.button.tpae-deal-btn:hover{background:#B91D15!important;}.notice.tpae-notice-show.tpae-bf-banner a.button.tpae-offer-btn:hover{background:#f3f3f3 !important;}</style>';
			?>
			<script>
                jQuery(document).on('click', '.tpae-bf-banner.tpae-notice-show .notice-dismiss', function(e) {
                    e.preventDefault();
                    ...
                    jQuery.ajax({
                        ...
                        type: 'POST',
                        data: {
                            action: 'tpae_blackfriday_dismiss_notice',
                            security: "<?php echo esc_attr( $nonce ); ?>",
                        },
                        success: function(response) {
                            jQuery('.tpae-bf-banner').hide();
                        }
                    });
                });
            </script>
		...
		/**
		 * It's is use for Save key in database for the TPAE Black friday Banner 
		 *
		 * @since 6.4.2
		 */
		public function tpae_blackfriday_dismiss_notice() {
			$get_security = ! empty( $_POST['security'] ) ? sanitize_text_field( wp_unslash( $_POST['security'] ) ) : '';
			
		
			if ( ! $get_security || ! wp_verify_nonce( $get_security, 'tpae-blackfriday-banner' ) ) {
	...
	Tpae_Blackfriday_Banner::instance();
}
